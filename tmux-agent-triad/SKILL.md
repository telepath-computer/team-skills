...
2. Create tmux window with that name (lowercase), with its cwd in the worktrees' parent folder — not the task worktree (codex-kind excepted; see above). State the worktree path in the agent's idle prompt instead.
