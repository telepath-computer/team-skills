...
**Suspect the rules that agents wrote.** There is an asymmetry in how agents handle rules: they are highly efficient at following litigious ones and extremely inefficient at authoring them unsupervised. An agent that writes a rule tends to give too little thought to its downstream effect on the litigious agents that will later apply it to the letter — often the same agent, later. The result is a sentence that reads like a settled principle and produces a confidently wrong action the first time a case arrives that its author did not picture: a cap that ends work which needed to continue, a stopping condition that halts anything slow. When you find a rule with no owner decision behind it, ask what single case produced it and what it now does to every other case. Rules written to make one moment tidy are among the most common things worth deleting.
