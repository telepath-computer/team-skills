...
- **When `watch` refuses, fix the cursor — don't reach for the pane.** On a session with many entries and no cursor it refuses rather than dumping (`refusing to print N chars from M entries`). That refusal is the safety cap working, but it returns nothing useful, and the tempting next move is `capture-pane`, which then quietly becomes the habit. Establish the cursor once with `superv watch <id> --reset --count 1` and routine observation is cheap again for the rest of the session.
